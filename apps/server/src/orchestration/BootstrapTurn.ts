...
/** Share thread/worktree bootstrap between interactive and durable delivery. */
export const makeBootstrapTurnDispatcher = Effect.fn("makeBootstrapTurnDispatcher")(function* (
  dispatchFromClient: OrchestrationEngine.OrchestrationEngineShape["dispatch"],
  durable = false,
) {
  ...
  const dispatchBootstrapTurnStart = (
    ...
    Effect.gen(function* () {
      ...
      const durableCommandId = (tag: string) =>
        durable
          ? Effect.succeed(CommandId.make(`dictation:${command.commandId}:${tag}`))
          : serverCommandId(tag);
      const { bootstrap: _bootstrap, ...finalTurnStartCommand } = command;
      let createdThread = false;
      let targetProjectId = bootstrap?.createThread?.projectId;
      let targetProjectCwd = bootstrap?.prepareWorktree?.projectCwd;
      let targetWorktreePath = existingWorktree ?? bootstrap?.createThread?.worktreePath ?? null;

      const cleanupCreatedThread = () =>
        createdThread && !durable
      ...
      const bootstrapProgram = Effect.gen(function* () {
        ...
        if (bootstrap?.prepareWorktree && !existingWorktree) {
          ...
          yield* dispatchFromClient({
            ...
            commandId: yield* durableCommandId("bootstrap-thread-meta-update"),
            threadId: command.threadId,
        ...
        if (!durable || !existingWorktree) yield* runSetupProgram();
  ...
  return dispatchBootstrapTurnStart;
});
